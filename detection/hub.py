...
def _git_archive_link(repo_owner, repo_name, ref):
    ...
    return "https://github.com/{0}/{1}/zipball/{2}".format(repo_owner,repo_name,ref)
...
def _parse_repo_info(github):
    ...
    if ref is None:
        ...
        try:
            ...
            with urlopen("https://github.com/{0}/{1}/tree/main/").format(repo_owner,repo_name):
                ref = 'main'
...
def _validate_not_a_forked_repo(repo_owner, repo_name, ref):
    ...
    for url_prefix in (
            'https://api.github.com/repos/{0}/{1}/branches'.format(repo_owner,repo_name),
            'https://api.github.com/repos/{0}/{1}/tags'.format(repo_owner,repo_name)):
        page = 0
        while True:
            page += 1
            url = '{0}?per_page=100&page={1}'.format(url_prefix,page)
    ...
    raise ValueError('Cannot find {0} in https://github.com/{1}/{2}. '
                     'If it\'s a commit from a forked repo, please call hub.load() with forked repo directly.'.format(ref, repo_owner,repo_name))
    ...
    if use_cache:
        ...
        try:
            ...
            if err.code == 300:
                ...
                warnings.warn(
                    "The ref {0} is ambiguous. Perhaps it is both a tag and a branch in the repo? ".format(ref), "Torchhub will now assume that it's a branch. ", "You can disambiguate tags and branches by explicitly passing refs/heads/branch_name or ", "refs/tags/tag_name as the ref. That might require using skip_validation=True.")
                disambiguated_branch_ref = "refs/heads/{0}".format(ref)
...
def _check_repo_is_trusted(repo_owner, repo_name, owner_name_branch, trust_repo, calling_fn="load"):
    ...
    if trust_repo is None:
        if not is_trusted:
            warnings.warn(
                "You are about to download and run code from an untrusted repository. In a future release, this won't ","be allowed. To add the repository to your trusted list, change the command to {0}(..., ","trust_repo=False) and a command prompt will appear asking for an explicit confirmation of trust, ".format(calling_fn), "or {0}(..., trust_repo=True), which will assume that the prompt is to be answered with ".format(calling_fn), "'yes'. You can also use {}(..., trust_repo='check') which will only prompt for ".format(calling_fn), "confirmation if the repo is not already trusted. This will eventually be the default behaviour")
        return

    if (trust_repo is False) or (trust_repo == "check" and not is_trusted):
        response = input( "The repository {0} does not belong to the list of trusted repositories and as such cannot be downloaded. ".format(owner_name), "Do you trust this repository and wish to add it to the trusted list of repositories (y/N)?")
        if response.lower() in ("y", "yes"):
            ...
            raise ValueError("Unrecognized response {0}.".format(response))
...
def load(repo_or_dir, model, *args, source='github', trust_repo=None, force_reload=False, verbose=True,
    ...
    if source not in ('github', 'local'):
        raise ValueError(
            'Unknown source: "{0}". Allowed values: "github" | "local".'.format(source))
